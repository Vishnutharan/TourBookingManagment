...
namespace TourBookingManagment.Controllers
{
    ...
    public class BookingController : ControllerBase
    {
        ...
        public BookingController(AppDbContext context, ILogger<BookingController> logger, IEmailService emailService)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IActionResult> CreateBooking([FromBody] BookingDetails booking)
        {
            try
            {
                ...
                try
                {
                    await _emailService.SendBookingConfirmationAsync(newBooking.Id);
                }
                catch (Exception emailEx)
                {
                    _logger.LogError(emailEx, "Failed to send confirmation email, but booking was created successfully");
                    // Don't throw here - we still want to return the booking confirmation
                }
        ...
        // Other existing controller methods remain unchanged...
